...
class Model(nn.Module):
    ...
    def forward(self, x):
        context = x[0]  # 输入的句子，x是(x, seq_len, mask)
...
def convert(content):
    ...
    str2list = list(content) # 例如：abc变成['a','b','c']
    if len(str2list) <= 256:
        return content
    else:
        list2str = "".join(content[:256]) # 截断
        return list2str
    ...
    for line in data:
        ...
        if pad_size:
            if len(token) < pad_size:
                mask = [1] * len(token_ids) + [0] * (pad_size - len(token))  # [1,1,1,0],1标识原句，0标识补全
                token_ids += ([0] * (pad_size - len(token)))  # id用0补全
            else:
                mask = [1] * pad_size
...
def final_predict(config, model, data_iter):
    ...
    model.load_state_dict(torch.load(config.save_path, map_location=map_location)) # 加载自己训练好的模型参数
    model.eval()
...
def predict(text):
    ...
    test_data = load_dataset(text, config)  # (token_ids, int(0), seq_len, mask)
    test_iter = build_iterator(test_data, config)
    result = final_predict(config, model, test_iter) # 预测结果
    for i, j in enumerate(result):  # 预测结果格式化输出
